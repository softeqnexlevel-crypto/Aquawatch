  ...
  return (
    <div ref={ref} style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 200, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 32px rgba(0,0,0,0.15)", zIndex: 100, overflow: "hidden", padding: "6px 0" }}>
      <div style={{ padding: "10px 14px 8px", borderBottom: "1px solid var(--border)", marginBottom: 4 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "var(--foreground)" }}>J. Mwangi</div>
        <div style={{ fontSize: 10, color: "var(--muted-foreground)" }}>j.mwangi@utility.co.ke</div>
        <div style={{ fontSize: 9, marginTop: 3, color: "#22c55e", background: "rgba(34,197,94,0.1)", display: "inline-block", borderRadius: 3, padding: "1px 6px", fontWeight: 600 }}>Ops Manager</div>
      </div>
      {items.map(({ icon: Icon, label, danger }) => (
        <button
          key={label}
          onClick={onClose}
          style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "9px 14px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: danger ? "#ef4444" : "var(--foreground)", textAlign: "left" }}
          onMouseEnter={e => e.currentTarget.style.background = "var(--muted)"}
          onMouseLeave={e => e.currentTarget.style.background = "none"}
        >
          <Icon size={13} style={{ color: danger ? "#ef4444" : "var(--muted-foreground)" }} />
          {label}
        </button>
      ))}
    </div>
      ...
      >
        ...
        <div ref={userRef} style={{ position: "relative" }}>
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 500, color: "var(--foreground)", lineHeight: 1 }}>J. Mwangi</div>
              <div style={{ fontSize: 9, color: "var(--muted-foreground)", lineHeight: 1.2 }}>Ops Manager</div>
            </div>
