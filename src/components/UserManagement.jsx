...
// ✅ FIX: password now has a real default here, in state — not just a
// display-time fallback on the input's `value`. This is what the field
// actually starts as, so admins can select-all and type over it cleanly.
...
// Generates a reasonably strong random password an admin can hand to a new user.
...
function UserDrawer({ open, onClose, onSave, initial, isEdit = false }) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      // ✅ Fresh "New User" open — regenerate a fresh default password each
      // time rather than reusing whatever was left over from a previous
      // drawer session, so stale passwords never silently linger in state.
      ...
      const data = isEdit 
        ...
        : await apiCall('/auth/register', {
            ...
            body: JSON.stringify({
              ...
              lastName: form.lastName || '',
              email: form.email,
              // ✅ Sends exactly what the admin typed/generated — no more
              // silent fallback to a hardcoded default at save time.
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {!isEdit && (
            <div>
              <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 10, fontWeight: 600, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>
                <span>Password</span>
                <button
                  type="button"
                  ...
                  style={{
                    ...
                    color: "#0ea5e9", fontSize: 10, fontWeight: 600, textTransform: "none", letterSpacing: 0
                  }}
                >
                  <Dices size={11} /> Generate
                </button>
              </label>
              ...
              <div style={{ fontSize: 9, color: "var(--muted-foreground)", marginTop: 4 }}>
                Minimum 8 characters. Share this with the user securely — they can change it after first login.
              </div>
            </div>
          )}
